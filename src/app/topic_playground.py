...
from src.visualisation.dashboard_viz import reformat_data, extract_top_reviews, visualise_all_topics_playground, sentiment_pie_chart
import pandas as pd
    ...
    add_card(q, 'inserttopic', ui.form_card(
        ...
        items=[
            ui.textbox(name='playground_topic', 
                       ...
                       placeholder="Enter 'Baked goods' if you would like to explore more about this topic"),
...
async def page4_result(q: Q, topics):
        ...
        items=[ui.table(
            ...
            columns=[
                    ...
                    ui.table_column(name='tag', 
                                    ...
                                    min_width='140px', 
                                    cell_type=ui.tag_table_cell_type(name='tags', 
                                                                     tags=[ui.tag(label='POSITIVE', color='$mint')]
                                                                    )
                                    )
                    ],
            rows = [ui.table_row(name = str(i),
                                cells = [i,'POSITIVE' ]) for i in extracted_pos[0:10]],
            height='265px'),
            ]
        ))
    ...
    add_card(q, 'topic_data_neg_preview', ui.form_card(
        ...
        items=[ui.table(
            name='preview',
            columns=[ui.table_column(name='Reviews', 
                                    label='Reviews',
                                    sortable=True,
                                    max_width = '420px',
                                    #searchable = True,
                    ...
                    ui.table_column(name='tag', 
                                    ...
                                    min_width='140px', 
                                    cell_type=ui.tag_table_cell_type(name='tags', 
                                                                     tags=[ui.tag(label='NEGATIVE', color='$red')]
                                                                    )
                                    )
                    ],
            rows = [ui.table_row(name = str(i),
                                cells = [i, 'NEGATIVE']) for i in extracted_neg[0:10]],
            height='265px'),
            ]
        ))
