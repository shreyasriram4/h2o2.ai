...
async def page1_upload(q: Q):
    ...
    add_card(q, 'fileupload', ui.form_card(
            box=ui.box(zone='horizontal', height = '135px', width = '100%'),
            items=[
                ui.file_upload(name='file_upload', label='Upload reviews', compact=True,
                               multiple=False, file_extensions=['csv']),
...
async def page1_preview(q: Q, files):
    ...
    add_card(q, 'datapreview', ui.form_card(
        box=ui.box(zone='horizontal1', height='620px', width='100%'), 
        title='Preview top 30 entries',
        items=[ui.table(
            name='preview',
            columns=[ui.table_column(name=x, 
                                 label=x,
                                 sortable=True,
                                 max_width = '500px',
                                 searchable = True,
