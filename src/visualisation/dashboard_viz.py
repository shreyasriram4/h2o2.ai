...
def topics_line_chart_over_time(data):
    freq_df = data.groupby(['date', 'pred_topic_label'],
                           as_index=False).size()
    fig = px.area(freq_df, x="date", y="size", color="pred_topic_label",
...
def visualise_all_topics(data):
    return visualise_top_words(data, labels=CONFIG_PARAMS["labels"],
                               specific=False,
