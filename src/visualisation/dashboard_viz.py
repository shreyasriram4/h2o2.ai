...
def update_chart(fig):
    ...
    return fig.update(
        layout=go.Layout(margin=dict(t=40, r=0, b=40, l=0), legend=dict(
            yanchor="top", y=0.95, xanchor="left", x=0.01))
    )


def reformat_data(data):
    data = data.assign(sentiment=data.sentiment.map(
...
async def sentiment_pie_chart(data):
    colour = px.colors.qualitative.Safe[0:2]
...
async def topics_bar_chart(data):
    freq_df = data.groupby(
        ['pred_topic_label', 'sentiment'], as_index=False).size()
    freq_df['pct'] = freq_df.groupby('pred_topic_label', group_keys=False)[
        'size'].apply(lambda x: np.round(x*100/x.sum(), 1))
...
async def visualise_all_topics(data):
    return visualise_top_words(data,
                               labels=CONFIG_PARAMS["labels"],
                               specific=False,
                               custom_sw=CONFIG_PARAMS["custom_stopwords"])


async def visualise_all_topics(data, topic):
    return visualise_top_words(data, labels=topic,
                               specific=True,
                               custom_sw=CONFIG_PARAMS["custom_stopwords"])
...
async def extract_top_reviews(data, topic):
    ...
    topic_sliced = list(topic_df.sort_values(
        by="score").head(5)["partially_cleaned_text"])
    return topic_sliced
